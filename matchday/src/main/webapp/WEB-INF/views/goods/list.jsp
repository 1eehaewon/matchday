...
<div class="container text-center search-container">
	...
	<div class="col-l-20">
		<form method="get" action="search">
			<br> <span class="product-label">상품명</span> <input type="text"
				name="productname" value="${productname}" size="30"> <input
		...
		<div class="col-auto">
			<div class="dropdown">
				<button class="btn btn-secondary dropdown-toggle" type="button"
					...
					aria-expanded="false">기본상품순</button>
				<div class="dropdown-menu dropdown-menu-end"
					aria-labelledby="dropdownMenuButton">
					<a class="dropdown-item" href="#">기본상품순</a> <a
						class="dropdown-item" href="#">최신등록순</a> <a class="dropdown-item"
						href="#">높은가격순</a> <a class="dropdown-item" href="#">낮은가격순</a>
				</div>
			</div>
		</div>
	</div>
					...
					<div class="item-card" data-category="${row.category}">
						<c:choose>
							<c:when test="${not empty row.filename && row.filename != '-'}">
								...
								<a
									href="${pageContext.request.contextPath}/goods/detail?goodsid=${row.goodsid}">
									<img
									src="${pageContext.request.contextPath}/storage/${row.filename}"
									class="img-responsive margin" style="width: 100%">
								</a>
							</c:when>
							<c:otherwise>
								<%--<a href="${pageContext.request.contextPath}/goods/detail/${row.goodsid}"> --%>
								<a
									href="${pageContext.request.contextPath}/goods/detail?goodsid=${row.goodsid}">
									<img
									src="${pageContext.request.contextPath}/images/default_product_image.jpg"
						...
						<p>
							<a
								href="${pageContext.request.contextPath}/goods/detail?goodsid=${row.goodsid}">${row.productname}</a>
						</p>
						<p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const shopItems = document.querySelectorAll('.item-card');
